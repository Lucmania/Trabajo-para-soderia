...
async function cargarClientes() {
    try {
        ...
        renderClientes(); // Renderizar la lista de clientes habilitados
    } catch (error) {
...
function renderClientes() {
    ...
    clientesList.innerHTML = ''; // Limpiar la lista antes de renderizar
    ...
    if (clientes.length > 0) {
        ...
        table += `</tbody></table>`;
        clientesList.innerHTML = table; // Añadir la tabla al HTML
    }
}
...
async function disableCliente(index) {
    ...
    await fetch(`http://localhost:5000/api/clientes/${clienteId}/habilitar`, {
        ...
        body: JSON.stringify({ estado: 'deshabilitado' }), // Deshabilitar
    });
